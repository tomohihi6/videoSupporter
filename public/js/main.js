...
function getDisplayTime(time) {
  const displayTime =
    ('00' + time.minutes).slice(-2) + ':' + ('00' + time.seconds).slice(-2);
...
function addScriptItem(e) {
  ...
  const scriptItemContainer = createScriptItemElement(
    mouse,
    startTime,
    endTime
  );
...
function getScriptTime(e) {
  ...
  const startTime = timeConvert(
    (rect.left - canvasRect.left - adjCanvasX) / lineWidth
  );
  const endTime = timeConvert(
    (rect.right - canvasRect.left - adjCanvasX) / lineWidth
  );
...
function confirmSaveScript() {
  const result = window.confirm(
    'ファイルをデータベースにアップロードしますか？'
  );
  if (result) {
    saveEditData();
    uploadFileToFirebaseStorage(`${videoId}.js`);
  }
}

function gatherTextsFromScriptItems() {
  // srt.js記述用要素全てを取得
  let scriptItemContainers = document.getElementsByClassName(
    'scriptItemContainer'
  );
  // スクリプト記録用変数
  let texts = '';
  ...
  function deleteBlankLine(string) {
    const rgexp = new RegExp(/^(\r?\n)/gm);
    const s = string.replace(rgexp, '');
  ...
  scriptItemContainers.forEach((e, i) => {
    texts += i + '\n';
    const string = deleteBlankLine(e.childNodes[1].textContent);
    texts += string + '\n\n';
  });
...
function openModal() {
  ...
  $('#modal-texts').html(
    `アップロードが完了しました．\n作成したスクリプトの動作は<a href="${mainUrl}" style="color:#00bbdd" target="_blank" rel="noopener noreferrer">こちら</a>で確認できます．`
  );
...
function loadEditData() {
  const reviveScriptItems = (text, left, width) => {
    const scriptItemConteiner = document.createElement('div');
    scriptItemConteiner.setAttribute(
      'class',
      'scriptItemContainer ui-selectee'
    );
    scriptItemConteiner.style.position = 'absolute';
  ...
  const widths = localStorage
    .getItem(`${videoId}_widths`)
    .split('END_OF_ARRAY');
...
function saveEditData() {
  ...
  alert(
    'ブラウザに編集データを一時保存しました．ブラウザを閉じると削除されます．'
  );
  return null;
}
